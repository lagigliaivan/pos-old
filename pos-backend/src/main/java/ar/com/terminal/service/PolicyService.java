...
public class PolicyService {
    ...
    public Response addProfitPolicy(@ApiParam(value = "Message to add. \"id\" will be ignored", required = true) final ProfitPolicyDto policy ) {
    ...
    @ApiResponses(value = {@ApiResponse(code = HttpStatus.CREATED_201, message = MSG_OK_201),
            @ApiResponse(code = HttpStatus.INTERNAL_SERVER_ERROR_500, message = MSG_INTERNAL_SERVER_ERROR_500)})
    public Response addProductToPolicy(@PathParam("id") String policyId, @ApiParam(value = "List of products to be associated to the policy", required = true) final ListHolder productsId) {

        controller.addProductToPolicy(policyId, productsId.getList());
        return formatResponse(HttpStatus.CREATED_201, "http:\\urlpolicy", MediaType.TEXT_PLAIN);
    }
